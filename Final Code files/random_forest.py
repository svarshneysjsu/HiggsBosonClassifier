# %% [markdown]
# # Random Forest for Higgs Boson Detection
...
# 
# if TRAIN is True, this will train a Random Forest Pipeline Model using train csv
# 
# if TRAIN is False, this will load a trained Random Forest Pipeline Model from `models`
...
if TRAIN:
    ...
    classifier = RandomForestClassifier(featuresCol="features", labelCol="signal", numTrees=20, maxDepth=10)
    pipeline = Pipeline(stages=[vector_assembler, classifier])
    model = pipeline.fit(train_df)
    model.save(os.path.join(MODELS_FOLDER, "randomForest"))
else:
    model = PipelineModel.load(os.path.join(MODELS_FOLDER, "randomForest"))
