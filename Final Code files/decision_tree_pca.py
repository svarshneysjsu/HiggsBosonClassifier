# %% [markdown]
# # Decision Tree for Higgs Boson Detection
...
# 
# if TRAIN is True, this will train a Decision Tree Pipeline Model using train csv
# 
# if TRAIN is False, this will load a trained Decision Tree Pipeline Model from `models`
...
if TRAIN:
    ...
    if TUNE:
        ...
        model.save(os.path.join(MODELS_FOLDER, "decisionTreePCATuned"))
    else:
        model = pipeline.fit(train_df)
        model.save(os.path.join(MODELS_FOLDER, "decisionTreePCA"))
else:
    if TUNE:
        model = PipelineModel.load(os.path.join(MODELS_FOLDER, "decisionTreePCATuned"))
    else:
        model = PipelineModel.load(os.path.join(MODELS_FOLDER, "decisionTreePCA"))

# %%
print(
    f"Model Parameters -"
    + f" k[{model.stages[2].getK()}]"
    + f" impurity[{model.stages[3].getImpurity()}]"
    + f" maxDepth[{model.stages[3].getMaxDepth()}]"
    + f" minInstancesPerNode[{model.stages[3].getMinInstancesPerNode()}]"
)
...
feature_names = [f"pca{i}" for i in range(len(model.stages[0].getInputCols()))]
