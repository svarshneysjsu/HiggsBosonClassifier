...
if TRAIN:
    ...
    classifier = DecisionTreeClassifier(
        featuresCol="features", labelCol="signal", seed=42, maxDepth=10
    )
    ...
    if TUNE:
        paramGrid = (
            ...
            .addGrid(classifier.minInstancesPerNode, [10, 20])
            .build()
        )
        ...
        model.save(os.path.join(MODELS_FOLDER, "decisionTreeTuned"))
    else:
        model = pipeline.fit(train_df)
        model.save(os.path.join(MODELS_FOLDER, "decisionTree"))
else:
    if TUNE:
        model = PipelineModel.load(os.path.join(MODELS_FOLDER, "decisionTreeTuned"))
    else:
        model = PipelineModel.load(os.path.join(MODELS_FOLDER, "decisionTree"))

# %%
print(
    f"Model Parameters -"
    + f" impurity[{model.stages[1].getImpurity()}]"
    + f" maxDepth[{model.stages[1].getMaxDepth()}]"
    + f" minInstancesPerNode[{model.stages[1].getMinInstancesPerNode()}]"
)
